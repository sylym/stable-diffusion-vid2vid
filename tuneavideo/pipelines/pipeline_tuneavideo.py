...
from ..util import ddim_inversion, controlnet_image_preprocessing
...
class TuneAVideoPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def _encode_prompt(self, prompt, device, num_videos_per_prompt, do_classifier_free_guidance, negative_prompt=None, prompt_embeds: Optional[torch.FloatTensor] = None, negative_prompt_embeds: Optional[torch.FloatTensor] = None):
    ...
    def prepare_latents(self, ddim_prompt, scheduler_path, video_input_dataloader, latent_timestep, use_vid2vid, use_inv_latent, num_inv_steps, batch_size, num_channels_latents, video_length, height, width, dtype, device, generator, latents=None):
        ...
        if latents is None:
            ...
            if use_vid2vid or use_inv_latent:
                for _, batch_all in enumerate(video_input_dataloader):
                    batch = batch_all
                pixel_values = batch["pixel_values"].to(self.vae.dtype)
                video_length_in = pixel_values.shape[1]
                pixel_values = rearrange(pixel_values, "b f c h w -> (b f) c h w")
                init_latents = self.vae.encode(pixel_values.to(device)).latent_dist.sample()
                init_latents = rearrange(init_latents, "(b f) c h w -> b c f h w", f=video_length_in)
                init_latents = init_latents * 0.18215
                shape = init_latents.shape
                if use_inv_latent:
                    ddim_inv_scheduler = DDIMScheduler.from_pretrained(scheduler_path, subfolder='scheduler')
            ...
            if isinstance(generator, list):
                ...
                if use_vid2vid and use_inv_latent:
                    for i in range(batch_size):
                        ...
                        latents.append(self.scheduler.add_noise(init_latents.to(rand_device), noise, latent_timestep))
                elif use_vid2vid:
                    for i in range(batch_size):
                        noise = torch.randn(shape, generator=generator[i], device=rand_device, dtype=dtype)
                        latents.append(self.scheduler.add_noise(init_latents.to(rand_device), noise, latent_timestep))
                elif use_inv_latent:
                    for i in range(batch_size):
                        latents.append(ddim_inversion(self, ddim_inv_scheduler, init_latents, num_inv_steps=num_inv_steps, prompt=ddim_prompt)[-1].to(self.vae.dtype))
                else:
                    for i in range(batch_size):
